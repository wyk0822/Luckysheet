        ...
        if (item.hasOwnProperty('chart') && item.chart.length > 0) {
            ...
            item.chart.forEach((chartInfo) => {
                ...
                chartInstance.resize({
                    width: chartInfo.width, height: chartInfo.height, animation: {
                        duration: 0
                    }
                })
...
export function setHyperlink(rowIndex, colIndex, linkData) {
    ...
    let currentHyperlink = sheet.hyperlink
    // console.log(sheet, sheet.hyperlink);
    currentHyperlink[rowIndex + "_" + colIndex] = linkData
    // {
    //     linkType: 'external', //链接类型 external外部链接，internal内部链接
    //     linkAddress: 'http:www.baidu.com//',  //链接地址 网页地址或工作表单元格引用
    //     linkTooltip: '哈师大睡个好觉',  //提示
    // };
